...
int main(int argc, char *argv[])
{
	...
	for(int j = 0; j < 80; j++)
	{
		im[j] = load_image_color(file_name[j], 224, 224);
	}
...
#if 0
	Ntask *nt0 = ntask_partition_range_create("nt0", RESNET18_NPU_DYNAMIC, 0, 3, ntask_input_handler, ntask_output_handler);
	np->contain(np, 1, nt0);
#endif
#if 1
...
//		int* num = (int*)(nt3->get_memory_pointer(nt3)+output_offset);

		printf("> prediction ends -> took %f seconds\n", what_time_is_it_now() - time);

		print_label(name, *num);

		label = get_label(alphabet, name[*num], (im[j].h*.03));

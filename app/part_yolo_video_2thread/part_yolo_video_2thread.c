...
static int inference_and_show(Nprocess *np, char *name)
{
	...
	Image *im_out = (Image *)(nt2->get_memory_pointer(nt2)+output_offset);
        int c = show_image(*im_out, name, 1);
        if (c != -1)
...
void *thread_run(void *args)
{
	...
	char name[10];
	sprintf(name, "Demo%d", id);

        make_window(name, 1352/DIVIDER, 1013/DIVIDER, fullscreen);
	...
	//nt->set_priority(nt, 10);
        ...
        while (!demo_done)
        {
                demo_done = inference_and_show(np, name);
        }
...
int main(int argc, char *argv[])
{
	...
	int id1 = 0, id2 = 1;
	if (pthread_create(&thread, 0, thread_run, &id2))
	{
